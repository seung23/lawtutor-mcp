...
102건 독립 검증 평가셋 기준 (top_k=5). 정답은 시스템 외부(법령 원문·교과서 대표 판례)에서 설정. 평가 방법론은 [docs/EVALUATION.md](docs/EVALUATION.md) 참조.
...
| 도구 | n | Recall@5 | MRR |
|------|---|----------|-----|
| 법령 의미 검색 | 40 | 0.90 | 0.74 |
| 법령 정확 조회 | 12 | 1.00 | 1.00 |
| 판례 의미 검색 | 20 | 0.40 | 0.30 |
| 헌재결정 의미 검색 | 14 | 0.36 | 0.21 |
| 해석례 검색 | 8 | 1.00 | 1.00 |
| 사건번호 조회 | 8 | 1.00 | 1.00 |

법령 검색과 정확 조회는 안정적인 성능. 판례·결정례 의미 검색은 579K/99K 규모 컬렉션에서 교과서 대표 판례 1건을 top-5에 포함시키는 것이 구조적으로 어려워 Recall이 낮음. 상세 분석은 [평가 보고서](docs/EVALUATION.md#53-v2v3-recall-하락-원인-분석) 참조.
